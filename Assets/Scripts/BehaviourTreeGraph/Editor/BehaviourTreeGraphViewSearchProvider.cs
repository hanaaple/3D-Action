        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
...

            {
                ...
                foreach (var type in types)
                {
                    ...
                    NodeInfoAttribute attribute = type.GetCustomAttribute(typeof(NodeInfoAttribute)) as NodeInfoAttribute;
                    
                    if (attribute != null)
                    {
                        elements.Add(new SearchContextElement(type, $"DecoratorNode/{attribute.actionType}/{type.Name}"));
                    }
                    else
                    {
                        elements.Add(new SearchContextElement(type, $"DecoratorNode/{type.Name}"));
                    }
                }
            }

            {
                ...
                foreach (var type in types)
                {
                    ...
                    NodeInfoAttribute attribute =
                        type.GetCustomAttribute(typeof(NodeInfoAttribute)) as NodeInfoAttribute;
                    if (attribute != null)
                    {
                        elements.Add(new SearchContextElement(type,
                            $"ActionNode/{attribute.actionType}/{type.Name}"));
                    }
                    else
                    {
                        elements.Add(new SearchContextElement(type,
                            $"ActionNode/{type.Namespace}/{type.Name}"));
                    }
                }
            }
