...
namespace BehaviourTreeGraph.Runtime
{
    ...
    public abstract class BehaviourTreeGraphNode : ScriptableObject
    {
        ...
        public virtual void Initialize(string nodeName = "", string id = "")
        {
        ...
        {
            if (this is DecoratorNode decoratorNode)
            {
                decoratorNode.child = null;
            }
            else if (this is RootNode rootNode)
            {
                rootNode.child = null;
            }
            else if (this is CompositeNode compositeNode)
            {
                compositeNode.children.Clear();
            }
            nodeState = NodeState.Waiting;
            started = false;
        }
        ...
        public void Traverse(Action<BehaviourTreeGraphNode> visitor)
        {
            visitor?.Invoke(this);
